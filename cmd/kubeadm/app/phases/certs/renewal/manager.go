...
import (
	...
	kubeadmconstants "github.com/gzlj/kubeadm/cmd/kubeadm/app/constants"
...
func (rm *Manager) RenewUsingLocalCA(name string) (bool, error) {
	...
	// update ca
	//caCfg := certToConfig(caCert)
	//signingCert, err := cert.NewSelfSignedCACert(cert.Config{CommonName: "e2e-server-cert-ca"}, signingKey)

	//newCaCert, err := k8scert.NewSelfSignedCACert(*caCfg,caKey)
	//handler.readwriter.Write(newCaCert, caKey)




	newCert, newKey, err := NewFileRenewer(caCert, caKey).Renew(cfg)
	//newCert, newKey, err := NewFileRenewer(newCaCert, caKey).Renew(cfg)
